---
title: "Práctica Limpieza y análisis de datos"
author: "Oscar Augusto Diaz Triana"
date: "`r Sys.Date()`"
output: pdf_document
---
...
corazon.std <- corazon[corazon$aspiration == "std",]
corazon.turbo <- corazon[corazon$aspiration == "turbo",]

# Agrupación por estilo de coche
corazon.convertible <-
corazon[corazon$body.style == "convertible",]
corazon.hatchback <-
corazon[corazon$body.style == "hatchback",]
corazon.sedan <-
corazon[corazon$body.style == "sedan",]
corazon.wagon <-
corazon[corazon$body.style == "wagon",]
corazon.hardtop <-
corazon[corazon$body.style == "hardtop",]
```
...
```{r}
par(mfrow=c(2,2))
for(i in 1:ncol(corazon.numeric)) {
  title <- colnames(corazon.numeric[i])
  qqnorm(corazon.numeric[,i],main = paste("Diagrama Normal Q-Q para",title))
  qqline(corazon.numeric[,i],col="red")
  hist(corazon.numeric[,i],
        xlab= title,
        freq = FALSE,
        main = paste("Histograma de", title))
  shapiro.test(corazon.numeric[,i])
  print(paste("Prueba de ", title))
  print(shapiro.test(corazon.numeric[,i]))
}
```
...
# Calcular el coeficiente de correlación para cada variable cuantitativa con respecto al campo "Outpu" - Posee enfermedad Cardiovascular
for (i in 2:(ncol(corazon.numeric))) {
  spearman_test = cor.test(corazon.numeric[,i],
                           corazon.numeric[,1],
                           method = "spearman",
                           exact = FALSE)
  ...
  rownames(corr_matrix)[nrow(corr_matrix)] <- colnames(corazon.numeric)[i]
}
...
Así, identificamos cuáles son las variables más correlacionadas con el el diagnostico en función de su proximidad con los valores -1 y +1. Teniendo esto en cuenta, queda patente cómo la variable más relevante en la fijación del dianostico es el **frecuencia cardíaca alcanzada** del paciente. (thalachh)
...
```{r}
corazon.femenino.presion <-
corazon[corazon$aspiration == "std",]$price
corazon.masculino.presion <-
corazon[corazon$aspiration == "turbo",]$price
```
